...
桌面端的录入、确认和执行页面采用浅绿与纸白配色，左侧导航下方展示参考图中白色毛茸茸形象重新生成的猫咪。图片为本地透明 PNG；窄屏切换为顶部导航时隐藏形象，为操作留出空间。输入框、按钮具有键盘焦点提示；阻碍弹窗支持 Tab 循环和 Escape 关闭。

形象素材位于 `src/renderer/public/fluffy-cat.png`；生成方式与完整提示词见 [形象生成记录](../docs/mascot-generation.md)。
