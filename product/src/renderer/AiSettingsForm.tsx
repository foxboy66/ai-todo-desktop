...
export function AiSettingsForm({ settings, onSaved, onClose }: {
  settings: AiSettings;
  ...
  return <form className="ai-settings-form" onSubmit={async (event) => {
    ...
    <h2>大模型设置</h2>
    <p>默认不使用大模型，新任务耗时为 30 分钟，可随时手动修改。</p>
    ...
    <div className="form-footer">
      ...
      <button type="submit" className="primary" disabled={saving}>{saving ? '正在保存…' : '保存设置'}</button>
    </div>
  </form>;
}
