...
import { buildSchedule, createDraftTask, defaultAvailability, formatScheduleLabel, formatTime, getCheckpoints, getReminderInterval, getScheduleSegments, parseTime, starterTasks, type AvailabilityBlock, type Priority, type ScheduledTask, type Task } from '@/shared/domain';
  ...
  return <div className="app-shell">
    ...
    <main className="main-content">
      ...
      <div className="content-wrap">
        ...
        {view === 'review' && <Review tasks={tasks} schedule={schedule} version={version} onUpdateTask={updateTask} onAddTask={addTask} onUpdateTime={updateTime} onRemoveTask={(id) => { setTasks((items) => items.filter((task) => task.id !== id)); setSchedule((items) => items.filter((task) => task.id !== id)); }} onConfirm={() => void confirmPlan()} onBack={() => setStep('capture')} />}
