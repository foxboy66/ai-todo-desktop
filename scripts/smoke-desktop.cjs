...
(async () => {
  ...
  try {
    ...
    assert.equal(await page.locator('.step-nav button').count(), 2);
    await page.getByRole('button', { name: '批量添加', exact: true }).click();
    assert.equal(await page.getByLabel('今天想完成什么？').inputValue(), '写测试报告；准备会议');
    ...
    await page.getByRole('button', { name: '上移 准备会议', exact: true }).click();
    await page.waitForFunction(async () => (await window.aiTodo.load()).tasks[0]?.title === '准备会议');
    assert.equal(await page.locator('.task-title-input').first().inputValue(), '准备会议');
    ...
    await page.locator('.sidebar').getByRole('button', { name: '执行跟进', exact: true }).click();
    await page.getByRole('heading', { name: '执行跟进' }).waitFor();
    ...
    // Verify daily task actions through the packaged renderer and real SQLite.
    await page.getByRole('button', { name: '标记为已完成', exact: true }).click();
    await page.waitForFunction(async () => (await window.aiTodo.load()).tasks.some(task => task.status === '已完成'));
    await app.close(); app = undefined;
    page = await launch();
    const afterRestart = await page.evaluate(() => window.aiTodo.load());
    assert.equal(afterRestart.tasks.filter(task => task.status === '已完成').length, 1);
    assert.equal(afterRestart.tasks.find(task => task.status === '已完成').progress, 100);
    await page.getByRole('button', { name: '前一天', exact: true }).click();
    await page.getByRole('heading', { name: '这一天的任务', exact: true }).waitFor();
    assert.equal(await page.locator('.todo-item').count(), 0);
    await page.getByLabel('快速添加任务').fill('昨日未完成的测试任务');
    await page.getByRole('button', { name: '添加任务', exact: true }).click();
    await page.getByRole('button', { name: '移到今天 昨日未完成的测试任务', exact: true }).waitFor();
    await page.getByRole('button', { name: '回到今天', exact: true }).click();
    await page.getByRole('heading', { name: '今天的任务', exact: true }).waitFor();
    assert.equal(await page.getByRole('button', { name: '编辑 昨日未完成的测试任务', exact: true }).count(), 0);
    await page.getByRole('button', { name: '前一天', exact: true }).click();
    await page.getByRole('button', { name: '移到今天 昨日未完成的测试任务', exact: true }).click();
    await page.getByRole('button', { name: '回到今天', exact: true }).click();
    await page.getByRole('button', { name: '编辑 昨日未完成的测试任务', exact: true }).waitFor();
    assert.equal((await page.evaluate(() => window.aiTodo.load())).tasks.length, 3);
    await page.screenshot({ path: path.join(root, 'test-results/desktop-daily.png') });
    ...
    console.log('PASS: packaged launch, real SQLite/IPC, offline 30 minutes, encrypted settings, restart, tray single-click restore/focus, task reordering, completion persistence, date isolation, manual carryover and clear data');
  } finally {
