...
export function DailyList({ tasks, day, today, busy, onAdd, onUpdate, onConfirm, onAvailability, plan, onCapture, confirmed, onExecute }: {
  ...
  return <section className="daily-list" aria-label="每日任务清单">
    <div className="daily-heading heading-row"><div><h1>{day === today ? '今天的任务' : '这一天的任务'}</h1>
      <p>{tasks.length ? `${tasks.length} 项待处理任务` : '当前没有待处理任务。记下一件要做的事，从这里开始。'}</p></div>
    ...
    {confirmed && day === today && tasks.length > 0 && <div className="execution-banner"><span><Check size={16} />计划已确认，可以开始执行。</span><button className="link-button" onClick={onExecute} disabled={busy}>进入执行<ArrowRight size={15} /></button></div>}
    ...
    {!filtered.length && <p className="list-empty">{query ? '没有匹配的任务，试试其他关键词。' : '当前没有待处理任务。新增任务后，可在这里调整时间并开始执行。'}</p>}
