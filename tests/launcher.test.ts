...
describe('Windows launcher', () => {
  it('uses Windows line endings and an ASCII PowerShell entry point', () => {
    ...
    expect(launcher.toString('ascii')).toContain('scripts\\launch\\launch-ai-todo.vbs')
    const vbsLauncher = readFileSync(resolve(__dirname, '../scripts/launch/launch-ai-todo.vbs'), 'ascii')
    expect(vbsLauncher).toContain('shell.Run command, 0, False')
    expect(vbsLauncher).toContain('launch-ai-todo.ps1')
    const powershellScript = readFileSync(resolve(__dirname, '../scripts/launch/launch-ai-todo.ps1'))
    ...
    expect(powershellText).toContain("Join-Path $PSScriptRoot '..\\..'")
